...
def render_executable_tests(
    ...
    api_identities = [
        f"{str(row.get('method', '')).upper()} {row.get('path', '')}"
        for row in semantic_model.get("apis", [])
        if isinstance(row, Mapping)
    ]
    execution_identities = list(dict.fromkeys([
    ...
    case_creation_identities = [
        identity for identity in execution_identities
        if identity.split(" ", 1)[0] == "POST"
        and "{" not in identity
        and identity.split(" ", 1)[1].rstrip("/").rsplit("/", 1)[-1] in {"cases", "disputes"}
    ]
    files: dict[str, str] = {
...
EXECUTION_IDENTITIES = {execution_identities!r}
CASE_CREATION_IDENTITIES = {case_creation_identities!r}

def _path(identity, case_id):
    return identity.split(" ", 1)[1].replace("{{case_id}}", case_id).replace("{{dispute_id}}", case_id)

def test_api_contract_contains_the_audited_model():
    required_identities = {api_identities!r}
    contract = json.loads((Path(__file__).resolve().parents[1] / "openapi/openapi.json").read_text(encoding="utf-8"))
    assert set(required_identities).issubset(contract["x-required-endpoints"])
    registered = {{f"{{method}} {{route.path}}" for route in app.routes for method in (route.methods or set())}}
    assert set(contract["x-required-endpoints"]).issubset(registered)
    assert SEMANTICS["authority"]["real_payment_calls"] == "disabled"

def test_every_api_identity_executes_through_the_runtime_application():
    client = TestClient(app)
    created = {{}}
    for identity in EXECUTION_IDENTITIES:
        method = identity.split(" ", 1)[0]
        case_id = "dispute-generated-test" if "/disputes" in identity else "case-generated-test"
        payload = {{"dispute_id": case_id}} if identity == "POST /v1/disputes" else {{"case_id": case_id}}
        response = client.request(method, _path(identity, case_id), json=payload)
    ...
    assert CASE_CREATION_IDENTITIES
    assert all(created[identity]["timeline"] == ["case_created"] for identity in CASE_CREATION_IDENTITIES)
    assert created["POST /v1/disputes"]["case_id"] == "dispute-generated-test"
    assert created["GET /v1/disputes"]["count"] >= 2

def test_api_missing_and_default_identifier_branches_are_truthful():
    client = TestClient(app)
    assert client.get("/v1/disputes/missing").json()["status"] == "not_found"
    creation_identity = CASE_CREATION_IDENTITIES[0]
    generated = client.post(creation_identity.split(" ", 1)[1], json={{}}).json()
    assert generated["case_id"].startswith("case-")
""",
