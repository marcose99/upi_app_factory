...
def main() -> int:
    ...
    start_text = (APP_ROOT / "scripts/start_local.sh").read_text(encoding="utf-8")
    if "generated_application.app.interfaces.api.main:app" not in start_text:
        errors.append("start_local.sh does not launch the hardened generated API")
    if "upi_dispute_app.main:app" in start_text:
        errors.append("start_local.sh still launches the legacy application")
