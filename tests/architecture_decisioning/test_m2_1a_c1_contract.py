...
CONTRACT_PATH = ROOT / "config/architecture_decisioning/kernel_contract.v1.json"
...
def test_contract_load_is_deterministic_and_complete() -> None:
    ...
    second = load()
    assert first == second
    assert first["schema_version"] == "upi-app-factory.architecture-decision-kernel-contract.v1"
