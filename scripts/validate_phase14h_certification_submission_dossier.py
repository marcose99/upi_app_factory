...
POLICY_PATH = Path("policies/phase14h_certification_authority_submission_dossier_policy.json")
DOC_PATH = Path("docs/phase14h/certification_authority_submission_dossier.md")
DOSSIER_PATH = Path("scripts/build_certification_authority_submission_dossier.py")
AUDIT_PATH = Path(
    "workspace/factory_generated/upi_dispute_resolution/"
    "lifecycle_artifacts/phase14h/certification_authority_submission_dossier_audit.json"
)
PHASE14G_LOOP = Path("scripts/build_authority_findings_remediation_loop.py")
...
def validate() -> list[str]:
    failures: list[str] = []
    for path in [POLICY_PATH, DOC_PATH, DOSSIER_PATH, AUDIT_PATH, PHASE14G_LOOP]:
        if not path.exists():
    ...
    if policy.get("schema_version") != "certification-authority-submission-dossier-policy.v1":
        failures.append("Invalid policy schema_version")
    if policy.get("mode") != "CERTIFICATION_AUTHORITY_SUBMISSION_DOSSIER":
    ...
    for section_id in DOSSIER_SECTIONS:
        if section_id not in policy.get("required_dossier_sections", []):
            failures.append(f"Policy missing dossier section: {section_id}")

    dossier = build_certification_authority_submission_dossier()
    if dossier.get("status") != READY:
        failures.append("Certification authority submission dossier should be ready")
    failures.extend(validate_certification_authority_submission_dossier(dossier))
    ...
    if not isinstance(boundary_value, list):
        failures.append("Dossier must list certification boundary")
    else:
        boundary_names = {str(item) for item in boundary_value}
        for item in CERTIFICATION_BOUNDARY:
            if item not in boundary_names:
                failures.append(f"Dossier missing boundary item: {item}")

    cli = subprocess.run(
        [sys.executable, str(DOSSIER_PATH), "--requirement-id", "upi_dispute_resolution.demo"],
        check=False,
        text=True,
        capture_output=True,
    )
    if cli.returncode != 0:
        failures.append("Certification authority submission dossier CLI should pass")
    elif READY not in cli.stdout:
        failures.append("Certification authority submission dossier CLI did not emit ready status")
...
def main() -> int:
    failures = validate()
    if failures:
        print("Phase 14H validation failed:")
        for failure in failures:
            print(f" - {failure}")
        return 1
    print("Phase 14H certification authority submission dossier artifacts validated.")
    return 0
