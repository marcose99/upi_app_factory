...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 10.1 official-source evidence artifacts."
    )
    parser.add_argument(
        "--output-dir",
        default=(
        ...
        help="Directory where Phase 10.1 artifacts will be written.",
    )
    parser.add_argument(
        "--app-id",
    ...
    written = generate_official_source_artifacts(
        Path(args.output_dir),
        app_id=args.app_id,
    )
    print("Generated Phase 10.1 official-source artifacts:")
    for path in written:
        print(f"- {path}")
    return 0
