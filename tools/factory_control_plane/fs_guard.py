...
class FilesystemGuard:
    ...
    def private_stage(self, root: DirectoryHandle, staging_name: str) -> DirectoryHandle:
    ...
    def promote(self, root: DirectoryHandle, staging_name: str) -> None:
        staging_fd = os.open(staging_name, _DIR_FLAGS, dir_fd=root.fd)
        try:
            # Validate complete source and destination trees before the first
            # destructive publication effect.
            ...
            for name in os.listdir(staging_fd):
                ...
                try:
                    opened = os.fstat(child_fd)
                    self._safe_kind(opened)
                    quarantine = f".capability-promote-{secrets.token_hex(16)}"
                    os.rename(name, quarantine, src_dir_fd=staging_fd, dst_dir_fd=staging_fd)
                    self._verify_entry(staging_fd, quarantine, opened)
                    _rename_noreplace(quarantine, name, staging_fd, root.fd)
                finally:
                    os.close(child_fd)
        except OSError as exc:
        ...
        try:
            ...
            raise ControlPlaneError("safe staged promotion cleanup denied") from exc
