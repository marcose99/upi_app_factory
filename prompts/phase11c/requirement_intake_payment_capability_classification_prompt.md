# Phase 11C Prompt: Requirement Intake and Payment Capability Classification

The factory must classify payment-domain requirements before generation.

The generated primary payment application is real local software. External
ecosystem applications are simulated.

The intake/classification process must:
1. Parse the requirement document front matter.
2. Validate the real-primary-app and simulated-ecosystem boundary.
3. Reject unsafe external connectivity, live payment execution, non-synthetic
   data, or unsupported certification/readiness claims.
4. Extract requirement IDs.
5. Classify payment capability.
6. Assign support level.
7. Produce a gap report.
8. Produce a generation contract only when safe.
...
## LLM Expense Tracking Requirement

Every governed application build must calculate and preserve LLM expense evidence.

The generation contract must require:
1. A project-supplied build-time LLM pricing configuration.
2. A per-call LLM expense ledger.
3. Token usage capture for every LLM call.
4. Cost calculation for every LLM call.
5. A consolidated final expense summary.
6. A human-readable final expense report.
7. The final expense summary to be the last LLM-dependent artifact.
8. No additional LLM calls after the final expense summary is emitted.

Required per-call ledger fields:
- call_id
- agent_name
- model
- input_tokens
...
The factory must not hard-code changing provider prices in prompts. Prices must
come from a project-supplied pricing configuration captured at the start of the
build.
