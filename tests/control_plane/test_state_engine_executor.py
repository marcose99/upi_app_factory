...
def test_hosted_bwrap_ubuntu24_uses_packaged_restrictive_apparmor_profile() -> None:
    root = Path(__file__).resolve().parents[2]
    workflow = (root / ".github/workflows/governed-ci.yml").read_text(encoding="utf-8")
    assert workflow.count("apt-get download apparmor-profiles") == 2
    assert workflow.count("usr/share/apparmor/extra-profiles/bwrap-userns-restrict") >= 2
    assert workflow.count("sudo apparmor_parser -r /etc/apparmor.d/bwrap-userns-restrict") == 2
    assert workflow.count("/proc/sys/kernel/apparmor_restrict_unprivileged_userns") == 4
    assert workflow.count("bwrap --die-with-parent --new-session --unshare-all --share-net") == 2
    for forbidden in (
