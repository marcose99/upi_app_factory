...
PHASE_PAYLOADS: dict[str, dict[str, str]] = {'46H': {'config/path_identity_contract.json': '{\n'
         ...
         'tools/transformation_controller/phase46h.py': 'from __future__ import '
                                                        ...
                                                        '    raise '
                                                        'SystemExit(main())\n'},
 '46I': {'config/technical_identity_contract.json': '{\n'
                                                    '  '
         ...
         'docs/phase46i/README.md': '# Phase 46I — Bounded technical namespace '
                                    ...
                                    '`upi_dispute_resolution_factory` as a governed\n'
         ...
         'tools/transformation_controller/phase46i.py': 'from __future__ import '
                                                        ...
                                                        '    raise '
                                                        'SystemExit(main())\n'},
 '46J': {'config/identity_migration_readiness.json': '{\n'
                                                     '  "certification_posture": '
                                                     '"CERTIFICATION_READY_NOT_CERTIFIED",\n'
                                                     '  "controls": {\n'
...
PREREQUISITE_ARTIFACTS: dict[str, str] = {'workspace/factory_generated/upi_dispute_resolution/lifecycle_artifacts/phase17/enterprise_autonomous_hardening_audit.json': 'a83b01caa3c2372786644e7d4e002a00b21333b4a1f35879e3e09714d857e230',
...
def build_evidence_index(project_root: Path) -> str:
    ...
    for relative in EVIDENCE_INPUTS:
        ...
        if not path.is_file():
            raise ValueError(
                f"Required migration evidence input is missing: {relative}"
            )
        data = path.read_bytes()
        records.append(
            {
...
def provision_prerequisites(project_root: Path) -> dict[str, Any]:
    ...
    for relative, expected_hash in PREREQUISITE_ARTIFACTS.items():
        ...
        if not source.is_file():
            raise ValueError(
                f"Missing ignored prerequisite in main worktree: {relative}"
            )
        data = source.read_bytes()
        observed = hashlib.sha256(data).hexdigest()
        if observed != expected_hash:
            raise ValueError(
                f"Prerequisite hash mismatch in main worktree: {relative}"
            )
        destination.parent.mkdir(parents=True, exist_ok=True)
        temporary = destination.with_suffix(
            destination.suffix + ".prerequisite.tmp"
        )
        ...
        if copied != expected_hash:
            raise ValueError(
                f"Copied prerequisite hash mismatch: {relative}"
            )
        records.append(
            {
                "path": relative,
...
def install_phase(phase: str, project_root: Path) -> dict[str, Any]:
    ...
    for relative in sorted(payload):
        ...
        destination.parent.mkdir(parents=True, exist_ok=True)
        temporary = destination.with_suffix(
            destination.suffix + f".{phase.lower()}.tmp"
        )
