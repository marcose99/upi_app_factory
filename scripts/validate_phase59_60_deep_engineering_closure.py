...
def validate_report(root: Path) -> dict[str, Any]:
    ...
    if report.get("status") != "completed":
        raise AssertionError(f"closure did not complete: {report.get('mandatory_gates')}")
    ...
    if depth.get("overall", 0) < 80 or depth.get("domain_fidelity", 0) < 16 or depth.get("security_privacy", 0) < 12 or depth.get("testing_depth", 0) < 12:
    ...
    if any(action not in report.get("actions_not_performed", []) for action in ["commit", "merge", "push", "tag", "release", "deploy"]):
    ...
    if "PROMOTION_DECISION=GO_FOR_HUMAN_REVIEW" not in env_text or "PROHIBITED_ACTIONS_PERFORMED=0" not in env_text:
